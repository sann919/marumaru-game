...
function say(text){if('speechSynthesis' in window){window.speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(text);u.lang='ja-JP';u.rate=.84;window.speechSynthesis.speak(u);}}
function beep(ok){try{audioCtx ||=new(window.AudioContext||window.webkitAudioContext)();if(audioCtx.state==='suspended')audioCtx.resume().catch(()=>{});const o=audioCtx.createOscillator(),g=audioCtx.createGain();o.connect(g);g.connect(audioCtx.destination);o.frequency.value=ok?730:240;g.gain.setValueAtTime(.07,audioCtx.currentTime);g.gain.exponentialRampToValueAtTime(.001,audioCtx.currentTime+.18);o.start();o.stop(audioCtx.currentTime+.18);}catch(e){}}
function cleanup(){
  timers.forEach(clearTimeout);timers.clear();guideTimer=null;
...
function renderWord(stage,index,announce=true){
  ...
  validateStage(stage);cleanup();stageIndex=index;step=0;phase='playing';
...
function pick(m){
  const el=$(m.id);if(phase!=='playing'||!el||el.disabled||!monsters.includes(m))return;
  cancelGuide();
  if(m.char===answer[step]){beep(true);captureMonster(m);toast('みつけた！ '+m.char+'！');step++;updateUI();
    if(step===answer.length){phase='celebrating';monsters.forEach(mon=>$(mon.id).disabled=true);later(celebrate,520);}
...
function melody(kind){
  ...
  notes.forEach((hz,i)=>later(()=>{try{audioCtx ||=new(window.AudioContext||window.webkitAudioContext)();if(audioCtx.state==='suspended')audioCtx.resume().catch(()=>{});const o=audioCtx.createOscillator(),g=audioCtx.createGain();o.type='sine';o.frequency.value=hz;o.connect(g);g.connect(audioCtx.destination);g.gain.setValueAtTime(.065,audioCtx.currentTime);g.gain.exponentialRampToValueAtTime(.001,audioCtx.currentTime+.3);o.start();o.stop(audioCtx.currentTime+.3);}catch(e){}},i*160));
}
function loadStage(index,announce=true){
  ...
  $('game').classList.toggle('boss-mode',stage.type==='boss');$('game').setAttribute('data-area',stage.areaId);$('game').style.setProperty('--area-dark',BACKGROUNDS[stage.areaId].color);
  $('bossIntro').hidden=true;$('bossWin').hidden=true;$('bossHud').hidden=stage.type!=='boss';
  if(stage.type==='boss'){
    const boss=bossForStage();bossQuestions=stage.challengeIds.map(id=>WORD_STAGES.find(w=>w.id===id));
    renderBossQuestion(false);phase='bossIntro';
...
function beginBoss(){if(phase!=='bossIntro')return;$('bossIntro').hidden=true;phase='playing';melody('intro');say('ことばで なかよく なろう！ '+bossQuestions[0].displayName+'をつくろう！ まずは、'+answer[0]+'を みつけよう！');}
function finishWord(){
  if(isBossStage()){
    if(bossQuestion===bossQuestions.length-1){finishBoss();return;}
    phase='questionComplete';$('message').classList.add('ready');$('resultDetail').textContent='なかよしパワーが ふえたよ！';say('できあがり！ なかよしパワーが ふえたよ！');
  }else{phase='complete';$('message').classList.add('ready');$('resultDetail').textContent='できあがり！';say('できあがり！');}
}
function nextStage(){
  if(isBossStage()&&phase==='questionComplete'){bossQuestion++;renderBossQuestion();return;}
  if(phase==='complete'&&stageIndex<STAGES.length-1)loadStage(stageIndex+1);
}
function showBossCard(){
  cleanup();phase='bossComplete';const boss=bossForStage(),fresh=awardCard(boss.cardId,STAGES[stageIndex].areaId),card=getCard(boss.cardId);
  ...
  $('bossCardNotice').textContent=fresh?'ボスカードをゲット！':'ボスカードをゲット！';
...
function openBook(){
  ...
  if(!$('cardBook').open)$('cardBook').showModal();
}
function renderBook(){
...
function showCard(id){
  const card=getCard(id),boss=BOSSES.find(b=>b.id===card.bossId);
  ...
  $('detailDescription').textContent=card.acquired?boss.description:'ことばを あつめて、このエリアの ボスと なかよく なろう！';
  $('bookBack').focus();
}
...
$('bossStart').addEventListener('click',beginBoss);
$('openBook').addEventListener('click',openBook);$('bossBook').addEventListener('click',openBook);
$('bookClose').addEventListener('click',()=>$('cardBook').close());
$('bookBack').addEventListener('click',()=>{renderBook();$('cardDetail').hidden=true;$('bookList').hidden=false;$('bookBack').hidden=true;});
$('bossContinue').addEventListener('click',()=>{if(phase==='bossComplete')loadStage(stageIndex<STAGES.length-1?stageIndex+1:0);});
$('restart').addEventListener('click',restart);$('again').addEventListener('click',restart);$('nextStage').addEventListener('click',nextStage);
$('fromStart').addEventListener('click',()=>{if(phase==='complete')loadStage(0);});
$('repeatGuide').addEventListener('click',()=>{cancelGuide();say(phase==='playing'?answer[step]+'を みつけよう！':(isBossStage()?bossQuestions[bossQuestion].displayName:STAGES[stageIndex].displayName)+'。できあがり！');});


validateContent();loadStage(0,false);guide("まずは、"+answer[0]+"を みつけよう！",700);
function finishBoss(){
  cleanup();phase='befriending';const boss=bossForStage();
...
$('skipFriend').addEventListener('click',completeFriendship);
function monsterDesign(char){
 const base=char.normalize('NFD')[0];
 const design=MONSTER_DEFINITIONS[base];if(!design)throw new Error('モンスター設定がありません: '+char);return design;
}
function drawMonster(art,char){
 const d=monsterDesign(char),[x,y,w,h]=d.rect,[bx,by,diameter]=d.badge;
...
function renderFoliage(stage){
 ...
 stage.monsters.forEach(m=>{
  const width=m.width||16,height=width*1.5,left=m.x-1,right=m.x+width+1,y=m.y+height*.86,bottom=m.y+height+1;
